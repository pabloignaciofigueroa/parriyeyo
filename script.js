...
function setupMotion(){
 ...
 mm.add('(min-width: 801px) and (prefers-reduced-motion: no-preference)',()=>{
   ...
   function activate(i){section.classList.toggle('is-warm',i===1);if(i===currentStep)return;steps[currentStep].classList.remove('active');currentStep=i;steps[i].classList.add('active');gsap.fromTo(steps[i],{y:22},{y:0,duration:.55,ease:'power3.out'});}
   ...
   return()=>{section.classList.remove('is-pinned','is-warm');steps.forEach((s,i)=>s.classList.toggle('active',i===0));};
 });
